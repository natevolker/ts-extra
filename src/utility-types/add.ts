...
import { NegativeNumberMap } from './negative-number-map';
...
type Convert<T extends string> =
  T extends keyof NegativeNumberMap
    ? NegativeNumberMap[T]
    : number;

/** Makes a type of sum result for A and B */
type AddHelper<A extends string | number | bigint, B extends string | number | bigint> =
...
type SubtractHelper<A extends string | number | bigint, B extends string | number | bigint> =
    (CheckIsNumeric<A> & CheckIsNumeric<B>) extends true
      ? B extends `-${infer Q}`
        ? AddHelper<A, Q>
        : AddHelper<A, `-${B}`>
